...
def log_check(name, table, passed, detail):
    dq_log.append(
        {"check_name": name, "table": table, "passed": passed, "detail": detail}
    )
...
customers = pd.read_csv(
    f"{RAW}/customers.csv", parse_dates=["dob", "signup_date", "record_date"]
)
...
log_check(
    ...
    "customers",
    True,
    f"dropped {before - len(customers)} exact duplicate rows",
)
...
log_check(
    ...
    "customers",
    True,
    f"{n_missing_city} missing cities set to 'Unknown' (row kept, not dropped)",
)
...
log_check(
    ...
    "dim_date",
    True,
    f"built {len(dim_date)} calendar day rows from {d_min.date()} to {d_max.date()}",
)
...
fact_transactions = clean_txn.merge(
    dim_account[["account_id", "account_sk"]], on="account_id", how="left"
)
fact_transactions = fact_transactions.merge(
    dim_branch[["branch_id", "branch_sk"]], on="branch_id", how="left"
)
fact_transactions = fact_transactions.merge(
    dim_product[["product_id", "product_sk"]], on="product_id", how="left"
)
fact_transactions["date_id"] = (
    fact_transactions["txn_date"].dt.strftime("%Y%m%d").astype(int)
)
